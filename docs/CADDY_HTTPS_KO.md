...
6. 독립 페이지를 `https://map.example.com/bmwc/chat`으로 열려면 `standalone-web.api-base-url`도 `/bmwc/api`로 설정합니다.
7. 업로드를 사용한다면 `upload.public-base-url`을 `/bmwc/api/uploads`로 설정합니다.
