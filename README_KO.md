...
## 4.5.0 업그레이드 참고 및 config 재생성 권장

4.5.0 계열은 DM/그룹채팅, 메타데이터 관리, 브라우저 알림, Web Push, PWA 이름, audit 로그, Discord 이모지 링크 처리 등 설정 변경이 많습니다. 4.3.x 기존 서버에서 모든 키를 수동 병합하기 어렵다면, 기존 `config.yml`을 백업한 뒤 새 config를 다시 생성하는 쪽이 더 안전합니다.

`config.yml`을 새로 만들어도 `history.db`, DM/그룹채팅 DB, 업로드 파일, 이모지 파일, audit 로그, 복사된 언어 파일, `web-push-vapid.properties` 같은 기존 자료는 삭제되지 않습니다. 새로 생성된 config는 `enabled: false`로 시작하므로, 설정을 검토하고 `enabled: true`로 바꾸기 전까지 웹/채팅 서비스, 정리 작업, DM/그룹채팅 저장소를 시작하지 않습니다.

권장 흐름:

```bash
# 먼저 Minecraft 서버를 종료합니다.
cd plugins/BlueMapWebChat
cp config.yml config.yml.4.3-backup
# 필요하면 전체 데이터 백업도 추가로 합니다.
# cp -a . ../BlueMapWebChat-backup-4.3
rm config.yml
# 서버를 한 번 시작하면 enabled: false 상태의 새 config.yml이 생성됩니다.
# 기존 커스텀 값을 옮긴 뒤 enabled: true로 바꿉니다.
```

4.5.0에서 확인하거나 병합해야 하는 주요 설정 예시:
...
`web-push.notification-title: ""`는 `standalone-web.app-name`을 사용한다는 뜻입니다. 푸시 제목을 고정하고 싶으면 이 값을 직접 넣으세요. `standalone-web.app-short-name`은 홈 화면/PWA처럼 공간이 좁은 곳에서 쓰는 짧은 이름입니다.
...
참고: `standalone-web.app-name`/`standalone-web.app-short-name`으로 모바일 홈 화면 웹앱 이름을 바꿀 수 있고, `web-push.notification-title`로 기본 푸시 제목을 바꿀 수 있습니다. `web-push.notification-title`을 비워두면 `standalone-web.app-name`을 사용합니다. iOS/iPadOS 푸시는 일반 탭이 아니라 standalone 페이지를 홈 화면에 추가한 웹앱에서만 시도하세요.
