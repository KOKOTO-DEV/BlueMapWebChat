...
Note: `standalone-web.app-name` / `standalone-web.app-short-name` 可更改移动端主屏幕 Web App 名称，`web-push.notification-title` 可更改默认推送标题。`web-push.notification-title` 留空时使用 `standalone-web.app-name`。iOS/iPadOS 请使用添加到主屏幕的 standalone 页面，而不是普通浏览器标签页。
